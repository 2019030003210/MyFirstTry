...
<script type="text/javascript">
    ...
    var use_count = change('<%=request.getAttribute("use_count") %>');
    var given_time = change('<%=request.getAttribute("given_time") %>');
    //初始化，默认显示标题，图例和xy空坐标轴
    myChart.setOption({
        title: {text: '统计指定地点的单车的每天平均使用次数'},
        tooltip: {},
        legend: {data: ['每天使用总时间(小时)'], show: true},
        xAxis: {data: given_time},
        yAxis: {},
        series: [{name: '使用次数', type: 'bar', data: use_count, barWidth: 30}]
    });
</script>
<div align="center">
